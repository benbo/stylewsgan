...
# You might want to set the visiblew gpus,
# in particular if you work on the same machine with multiple others
# os.environ["CUDA_VISIBLE_DEVICES"] = "0,1,2,3"

# Consider changing cache dirs
# os.environ['DNNLIB_CACHE_DIR'] = "/home/mycache/"
# os.environ['TORCH_HOME'] = "/home/mycache/"
...
def setup_training_loop_kwargs(
    ...
    args.loss_kwargs = dnnlib.EasyDict(
        class_name="training.loss.StyleWSGANLoss", r1_gamma=spec.gamma, cardinality=None
    )

    # label model
    args.LM_kwargs = dnnlib.EasyDict(
        class_name="training.networks.EncoderLabelmodel",
        Num_LFs=None,  # set in training_loop
        num_features=None,  # set in training_loop
        cardinality=None,  # set in training_loop
        hidden_dims=[128, 64, 32],
        class_balance=None,
        acc_activation="Sigmoid",
        modeltype="encoderX",
    )
    ...
    if cfg == "cifar":
        ...
        args.G_kwargs.mapping_kwargs.num_layers = (
            6  # set layers higher than in stylegan-ada
        )
        # !!Setting of embed_features is important for stable training dynamics!!
        # Should not be too large unless LFs have high coverage and are extremely accurate
        args.G_kwargs.mapping_kwargs.embed_features = 200

    if cfg == "lsun256":
        args.loss_kwargs.pl_weight = 0.0  # disable path length regularization
        args.loss_kwargs.style_mixing_prob = 0  # disable style mixing
        # !!Setting of embed_features is important for stable training dynamics!!
        # Should not be too large unless LFs have high coverage and are extremely accurate
        args.G_kwargs.mapping_kwargs.embed_features = 50
