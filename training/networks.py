...
class MappingNetwork(torch.nn.Module):
    def __init__(
        ...
        num_codes,  # Number of discrete ,latent codes dimensionality, 0 = no label.
        ...
        if num_codes > 0:
            self.embed = FullyConnectedLayer(num_codes, embed_features)
    ...
    def forward(
        ...
        with torch.autograd.profiler.record_function("input"):
            ...
            if self.num_codes > 0:
                misc.assert_shape(c, [None, self.num_codes])
...
class Generator(torch.nn.Module):
    def __init__(
        ...
        num_codes,  # Number of discrete latent codes (dimensionality).
        w_dim,  # Intermediate latent (W) dimensionality.
...
class DiscriminatorEpilogue(torch.nn.Module):
    def __init__(
        self,
        in_channels,  # Number of input channels.
        num_codes,  # number of discrete latent codes.
        resolution,  # Resolution of this block.
        img_channels,  # Number of input color channels.
        architecture="resnet",  # Architecture: 'orig', 'skip', 'resnet'.
        ...
        self.out_code = FullyConnectedLayer(in_channels, num_codes)

    def forward(self, x, img, force_fp32=False, ret_valid=True, ret_code=False):
        misc.assert_shape(
            x, [None, self.in_channels, self.resolution, self.resolution]
        )  # [NCHW]
        _ = force_fp32  # unused
        ...
        validity = self.out(features) if ret_valid else None
        if ret_code:
            predicted_code = self.out_code(features) if ret_code else None

            return validity, features, predicted_code
        else:
...
class Discriminator(torch.nn.Module):
    def __init__(
        ...
        self.map_code = True
        if self.map_code:
            self.fone = nn.Linear(num_codes, num_codes)

    def forward(
        self, img, ret_valid=True, ret_code=True, map_code=True, **block_kwargs
    ):
        x = None
        for res in self.block_resolutions:
            block = getattr(self, f"b{res}")
        ...
        validity, features, predicted_code = self.b4(
            x, img, ret_valid=ret_valid, ret_code=ret_code
        )
        fmapped_pred_code = None
        if ret_code:
            if self.map_code and map_code:
                fmapped_pred_code = self.fone(predicted_code)
            else:
                fmapped_pred_code = predicted_code
        return validity, features, predicted_code, fmapped_pred_code
...
class EncoderLabelmodel(nn.Module):
    ...
    def __init__(
        ...
        # add mapping functions to this model
        self.ftwo = nn.Linear(cardinality, cardinality)
    ...
    def forward(self, X, L, get_accuracies=False):
        ...
        yest_mapped = self.ftwo(Y)
        if get_accuracies:
            if self.modeltype == "vector":
                return Y, yest_mapped, self.acc.reshape(self.nlf)
            else:
                return Y, yest_mapped, acc
        return Y, yest_mapped
