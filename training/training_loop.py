...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    # Load data.
    images, labels, LFs, lfindices = zip(*[training_set[i] for i in grid_indices])
...
def training_loop(
    ...
    LM_kwargs={},  # Options for labelmodel network.
    ...
    common_kwargs = dict(
        num_codes=training_set.label_dim,
    ...
    # construct labelmodel
    LM_kwargs.Num_LFs = training_set.get_numLFs()
    LM_kwargs.num_features = D.b4.in_channels
    num_codes = training_set.label_dim
    LM_kwargs.cardinality = num_codes
    loss_kwargs.cardinality = num_codes
    loss_kwargs.num_LFs = training_set.get_numLFs()
    encoderlabelmodel = (
        dnnlib.util.construct_class_by_name(**LM_kwargs)
        .train()
        .requires_grad_(False)
        .to(device)
    )
    ...
    if rank == 0:
        ...
        c = torch.empty([batch_gpu, num_codes], device=device)
    ...
    opt_kwargs.lr = opt_kwargs.lr * 0.2
    opt = dnnlib.util.construct_class_by_name(
        itertools.chain(G.parameters(), D.parameters()), **opt_kwargs
    )
    phases += [
        dnnlib.EasyDict(name="Info", module=[G, D], opt=opt, interval=reg_interval)
    ]

    # add labelmodel phase
    reg_interval = 1
    opt_kwargs = dnnlib.EasyDict(G_opt_kwargs)
    opt_kwargs.lr = opt_kwargs.lr * 0.2
    opt = dnnlib.util.construct_class_by_name(
        itertools.chain(D.parameters(), encoderlabelmodel.parameters()), **opt_kwargs
    )
    phases += [
        dnnlib.EasyDict(
            name="Ymap", module=[D, encoderlabelmodel], opt=opt, interval=reg_interval
        )
    ]
    ...
    class_balance = torch.ones(num_codes, dtype=torch.float32)
    class_balance /= class_balance.sum()

    while True:
        # Fetch training data.
        with torch.autograd.profiler.record_function("data_fetch"):
            phase_real_img, phase_real_c, phase_lfs, phase_lf_indices = next(
            ...
            # transform Lfs to one-hot if necessary
            if not training_set.load_one_hot_lfs:
                phase_lfs = create_L_ind(phase_lfs, num_codes)
            phase_lfs = phase_lfs.to(device).split(batch_gpu)

            phase_lf_indices = phase_lf_indices.to(device).split(batch_gpu)

            # we use labels to track label model performance, but never use them for learning
            ...
            # TODO adjust samppling with class balance estimate for imbalanced weakly supervised datasets
            # sample uniformly
            all_gen_c = (
                torch.randint(
                    0, num_codes, (len(phases) * batch_size,), dtype=torch.int64
                )
                .pin_memory()
                .to(device)
            )

            # convert to one-hot, move to device
            all_gen_c_abs = all_gen_c
            all_gen_c_onehot = F.one_hot(all_gen_c, num_classes=num_codes)
            # split
            all_gen_c_onehot = [
                phase_gen_c.split(batch_gpu)
                for phase_gen_c in all_gen_c_onehot.split(batch_size)
            ]

            all_gen_c_abs = [
                phase_gen_c.split(batch_gpu)
                for phase_gen_c in all_gen_c_abs.split(batch_size)
            ]

        # Execute training phases.
        for phase, phase_gen_z, phase_gen_c, phase_gen_c_abs in zip(
            phases, all_gen_z, all_gen_c_onehot, all_gen_c_abs
        ):
            ...
            if isinstance(phase.module, list):
                for mitem in phase.module:
                    mitem.requires_grad_(True)
            else:
            ...
            for round_idx, (
                ...
                loss.accumulate_gradients(
                    ...
                    tick=cur_tick,
                    LFs=real_lfs,
                    filteridx=real_lf_idxs,
                )

            # Update weights.
            if isinstance(phase.module, list):
                for mitem in phase.module:
                    mitem.requires_grad_(False)
                    with torch.autograd.profiler.record_function(phase.name + "_opt"):
                        for param in mitem.parameters():
